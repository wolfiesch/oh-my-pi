...
- **Batch shape** (`task.batch` on): `{ context, tasks: item[] }` — one subagent per item, all run under the same fan-out rules; there is no top-level agent field. `context` is **required** shared background and cross-cutting constraints rendered into every spawned subagent's system prompt (`SHARED CONTEXT` section); it may constrain a child's assignment but cannot expand its executable scope. `agent` and `isolated` are per item, so one call may mix agent types.
...
| --- | --- | --- | --- |
| `context` | `string` | Yes (batch) | Shared background and cross-cutting constraints prepended to every spawn of the call via the subagent system prompt. Parent orchestration here is not child work unless that child's `task` explicitly includes it. Rejected when `task.batch` is off. |
...
- Nested concurrency: `task.maxNestedConcurrency` defaults to `-1` (inherit). Any non-negative value becomes each child session's `task.maxConcurrency`, so it also bounds that child's eval `parallel()` / `pipeline()` helpers. The limit is per child session, not a global cap across the full task tree.
