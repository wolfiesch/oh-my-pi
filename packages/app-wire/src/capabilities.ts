...
export const DEVICE_CAPABILITIES = [
  "sessions.read", "sessions.prompt", "sessions.control", "sessions.manage", "bash.run", "term.open",
  "term.input", "term.resize", "files.read", "files.write", "files.list", "files.diff", "agents.control",
  "audit.read", "config.read", "config.write", "catalog.read", "preview.read", "preview.control",
] as const;
...
export function decodeCapabilities(value: unknown, path = "capabilities"): Capabilities {
  ...
  for (const [i, capability] of client.entries()) if (!(DEVICE_CAPABILITIES as readonly string[]).includes(capability)) fail("INVALID_FRAME", "unknown device capability", `${path}.client[${i}]`);
  if (server !== undefined) for (const [i, capability] of server.entries()) if (!(DEVICE_CAPABILITIES as readonly string[]).includes(capability)) fail("INVALID_FRAME", "unknown device capability", `${path}.server[${i}]`);
...
export function decodeNegotiatedFeatureList(value: unknown, path: string): ProtocolFeature[] {
  ...
  for (const [i, feature] of result.entries()) if (!(PROTOCOL_FEATURES as readonly string[]).includes(feature)) fail("INVALID_FRAME", "unknown protocol feature", `${path}[${i}]`);
  return result as ProtocolFeature[];
}
export function isCapability(value: unknown): value is DeviceCapability { return typeof value === "string" && (DEVICE_CAPABILITIES as readonly string[]).includes(value); }
export function isProtocolFeature(value: unknown): value is ProtocolFeature { return typeof value === "string" && (PROTOCOL_FEATURES as readonly string[]).includes(value); }
