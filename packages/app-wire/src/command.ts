...
"session.state.get": { capability: "sessions.read", scope: "session", revision: "none", revisionOwner: "none", confirmation: "none" },
"session.steer": { capability: "sessions.prompt", scope: "session", revision: "optional", revisionOwner: "session", confirmation: "none" },
"session.followUp": { capability: "sessions.prompt", scope: "session", revision: "optional", revisionOwner: "session", confirmation: "none" },
"session.rename": { capability: "sessions.manage", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.retry": { capability: "sessions.control", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.compact": { capability: "sessions.control", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.pause": { capability: "sessions.control", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.resume": { capability: "sessions.control", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.model.set": { capability: "sessions.manage", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.thinking.set": { capability: "sessions.manage", scope: "session", revision: "required", revisionOwner: "session", confirmation: "none" },
"session.ui.respond": { capability: "sessions.prompt", scope: "session", revision: "optional", revisionOwner: "session", confirmation: "none" },
...
function strictArgs(value: unknown, allowed: readonly string[]): Record<string, unknown> { const out = args(value); const expected = new Set(allowed); for (const key of Object.keys(out)) if (!expected.has(key)) fail("INVALID_FRAME", "unknown command argument", `args.${key}`); return out; }
function leasedArgs(value: unknown, allowed: readonly string[]): Record<string, unknown> { const x = strictArgs(value, [...allowed, "leaseId"]); if (x.leaseId !== undefined) leaseId(x.leaseId, "args.leaseId"); return x; }
function noArgs(value: unknown): CommandArguments { return strictArgs(value, []); }
function decodeMessage(value: unknown): CommandArguments { const x = leasedArgs(value, ["message"]); controlFree(x.message, "args.message", MAX_FILE_BYTES); return x; }
function decodeSessionUiResponse(value: unknown): CommandArguments { const x = args(value); const keys = Object.keys(x); if (!Object.hasOwn(x, "requestId") || (keys.length !== 2 && keys.length !== 3) || (keys.length === 3 && !Object.hasOwn(x, "leaseId"))) fail("INVALID_FRAME", "ui response requires requestId and exactly one payload", "args"); if (x.leaseId !== undefined) leaseId(x.leaseId, "args.leaseId"); controlFree(x.requestId, "args.requestId", 256); const payload = keys.filter(key => key !== "requestId" && key !== "leaseId"); const key = payload[0]; if (key === "value") controlFree(x.value, "args.value", MAX_FILE_BYTES); else if (key === "confirmed") { if (typeof x.confirmed !== "boolean") fail("INVALID_FRAME", "confirmed must be boolean", "args.confirmed"); } else if (key === "cancelled") { if (x.cancelled !== true) fail("INVALID_FRAME", "cancelled must be true", "args.cancelled"); } else fail("INVALID_FRAME", "unknown ui response payload", `args.${key}`); return x; }
...
function decodeSettingsResult(value: unknown): CommandResult {
  ...
  for (const [rawPath, rawMetadata] of Object.entries(source)) {
    ...
    if (
      item.sensitive === true &&
      (Object.hasOwn(item, "default") || Object.hasOwn(item, "effective"))
    )
      fail(
        "INVALID_FRAME",
        "sensitive setting metadata must omit values",
...
function decodeBooleanResult(value: unknown, key: string): CommandResult { const x = result(value); const keys = Object.keys(x); if (keys.length !== 1 || keys[0] !== key || typeof x[key] !== "boolean") fail("INVALID_FRAME", "invalid command result", `result.${key}`); return { [key]: x[key] }; }
function decodeAcceptedResult(value: unknown): CommandResult { return decodeBooleanResult(value, "accepted"); }
function decodeSessionState(value: unknown): CommandResult { return decodeSessionStateResult(value) as unknown as CommandResult; }
function decodePauseResult(value: unknown): CommandResult { const x = result(value); if (Object.keys(x).length !== 2 || typeof x.paused !== "boolean" || typeof x.changed !== "boolean") fail("INVALID_FRAME", "invalid pause result", "result"); return { paused: x.paused, changed: x.changed }; }
export const COMMAND_ARGUMENT_DECODERS:Readonly<Record<string,(value:unknown)=>CommandArguments>>={
  "host.list":args,"session.list":args,"session.create":value=>{const x=args(value);if(x.cwd!==undefined)fail("UNSAFE_PATH","cwd is not accepted; use projectId","args.cwd");projectId(x.projectId,"args.projectId");if(x.title!==undefined)boundedText(x.title,"args.title",512);return x;},"session.attach":value=>{const x=args(value);if(x.cursor!==undefined)decodeCursor(x.cursor,"args.cursor");return x;},"session.prompt":value=>decodeSessionPromptArguments(value) as unknown as CommandArguments,"session.state.get":noArgs,"session.steer":decodeMessage,"session.followUp":decodeMessage,"session.rename":value=>{const x=leasedArgs(value,["name"]);controlFree(x.name,"args.name",512);return x;},"session.retry":value=>leasedArgs(value,[]),"session.compact":value=>{const x=leasedArgs(value,["instructions"]);if(x.instructions!==undefined)controlFree(x.instructions,"args.instructions",MAX_FILE_BYTES);return x;},"session.pause":value=>leasedArgs(value,[]),"session.resume":value=>leasedArgs(value,[]),"session.model.set":value=>{const x=leasedArgs(value,["provider","modelId"]);controlFree(x.provider,"args.provider",256);controlFree(x.modelId,"args.modelId",256);return x;},"session.thinking.set":value=>{const x=leasedArgs(value,["level"]);const level=controlFree(x.level,"args.level",64);if(!["inherit","off","minimal","low","medium","high","xhigh","max"].includes(level))fail("INVALID_FRAME","invalid thinking level","args.level");return x;},"session.ui.respond":decodeSessionUiResponse,"session.cancel":value=>leasedArgs(value,[]),"session.close":value=>leasedArgs(value,[]),
...
export const COMMAND_RESULT_DECODERS:Readonly<Record<string,(value:unknown)=>CommandResult>>={
  "host.list":decodeSessions,"session.list":decodeSessions,"session.create":decodeCreate,"session.attach":decodeAttach,"session.prompt":value=>boolField(value,"accepted"),"session.state.get":decodeSessionState,"session.steer":decodeAcceptedResult,"session.followUp":decodeAcceptedResult,"session.rename":value=>decodeBooleanResult(value,"renamed"),"session.retry":value=>decodeBooleanResult(value,"retried"),"session.compact":value=>decodeBooleanResult(value,"compacted"),"session.pause":decodePauseResult,"session.resume":value=>{const x=result(value);if(Object.keys(x).length!==2||typeof x.resumed!=="boolean"||typeof x.paused!=="boolean")fail("INVALID_FRAME","invalid resume result","result");return {resumed:x.resumed,paused:x.paused};},"session.model.set":decodeAcceptedResult,"session.thinking.set":decodeAcceptedResult,"session.ui.respond":decodeAcceptedResult,"session.cancel":value=>boolField(value,"cancelled"),"session.close":value=>boolField(value,"closed"),"files.read":value=>{const x=result(value);boundedText(x.content,"result.content",MAX_FILE_BYTES);return x;},"files.write":result,"files.patch":result,"files.list":decodeEntries,"files.diff":value=>{const x=result(value);boundedText(x.diff,"result.diff",MAX_FILE_BYTES);return x;},"review.read":result,"review.apply":result,"agent.cancel":value=>boolField(value,"cancelled"),"bash.run":result,"term.open":decodeTerminalResult,"audit.read":decodeAuditResult,"audit.tail":decodeAuditResult,"config.write":value=>metadata(value,"result"),"settings.read":decodeSettingsResult,"settings.write":value=>metadata(value,"result"),"catalog.get":decodeCatalogResult,"host.watch":decodeWatchResult,"session.watch":decodeWatchResult,"controller.lease.acquire":decodeLeaseResult,"controller.lease.renew":decodeLeaseResult,"controller.lease.release":decodeLeaseResult,"prompt.lease.acquire":decodeLeaseResult,"prompt.lease.renew":decodeLeaseResult,"prompt.lease.release":decodeLeaseResult,"preview.launch":result,"preview.state":result,"preview.navigate":result,"preview.capture":decodePreviewCaptureResult,
};
