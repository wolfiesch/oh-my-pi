import { hostId, sessionId, type HostId, type SessionId } from "./ids.ts";
import { boundedArray, boundedMap, inputObject, string } from "./guards.ts";
import { MAX_FILE_BYTES, PROTOCOL_VERSION } from "./limits.ts";
import { fail } from "./errors.ts";

export interface FileFrame {
  v: typeof PROTOCOL_VERSION; type: "files"; hostId: HostId; sessionId: SessionId;
  path: string; content?: string; truncated?: boolean;
}
export interface ReviewFrame {
  v: typeof PROTOCOL_VERSION; type: "review"; hostId: HostId; sessionId: SessionId;
  reviewId: string; status: string; findings: Record<string, unknown>[];
}
export function decodeFiles(input: unknown): FileFrame {
  const frame = inputObject(input);
  if (frame.v !== PROTOCOL_VERSION) fail("MISSING_VERSION", `expected ${PROTOCOL_VERSION}`, "v");
  if (frame.type !== "files") fail("INVALID_FRAME", "expected files frame", "type");
  hostId(frame.hostId); sessionId(frame.sessionId); string(frame.path, "path", 4096);
  if (frame.content !== undefined) string(frame.content, "content", MAX_FILE_BYTES);
  if (frame.truncated !== undefined && typeof frame.truncated !== "boolean") fail("INVALID_FRAME", "truncated must be boolean", "truncated");
  return frame as unknown as FileFrame;
}
export function decodeReview(input: unknown): ReviewFrame {
  const frame = inputObject(input);
  if (frame.v !== PROTOCOL_VERSION) fail("MISSING_VERSION", `expected ${PROTOCOL_VERSION}`, "v");
  if (frame.type !== "review") fail("INVALID_FRAME", "expected review frame", "type");
  hostId(frame.hostId); sessionId(frame.sessionId); string(frame.reviewId, "reviewId", 256); string(frame.status, "status", 64);
  const findings = boundedArray(frame.findings, "findings");
  for (let i = 0; i < findings.length; i++) boundedMap(findings[i], `findings[${i}]`);
  return frame as unknown as ReviewFrame;
}
