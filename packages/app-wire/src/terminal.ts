...
export function decodeTerminal(input: unknown): TerminalFrame {
  ...
  hostId(frame.hostId); sessionId(frame.sessionId); terminalId(frame.terminalId); string(frame.stream, "stream", 32);
  if (frame.data !== undefined) string(frame.data, "data", MAX_TERMINAL_OUTPUT_BYTES);
  if (frame.exitCode !== undefined && (typeof frame.exitCode !== "number" || !Number.isSafeInteger(frame.exitCode))) fail("INVALID_FRAME", "exitCode must be safe integer", "exitCode");
