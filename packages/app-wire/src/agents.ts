...
export function decodeAgent(input: unknown): AgentFrame {
  ...
  hostId(frame.hostId); sessionId(frame.sessionId); agentId(frame.agentId); string(frame.state, "state", 64);
  if (frame.progress !== undefined && (typeof frame.progress !== "number" || !Number.isFinite(frame.progress) || frame.progress < 0 || frame.progress > 1)) fail("BOUNDS", "progress must be between zero and one", "progress");
