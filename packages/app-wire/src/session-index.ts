...
import { boundedArray, boundedMap, boundedMetadata, inputObject, optionalString, controlFree, isSecretLikeKey } from "./guards.ts";
...
export interface SessionsFrame { v: typeof PROTOCOL_VERSION; type: "sessions"; cursor: Cursor; sessions: SessionRef[] }
...
export function decodeSessions(input: unknown): SessionsFrame { const frame = inputObject(input); if (frame.v !== PROTOCOL_VERSION) fail("MISSING_VERSION", `expected ${PROTOCOL_VERSION}`, "v"); if (frame.type !== "sessions") fail("INVALID_FRAME", "expected sessions frame", "type"); const cursor = decodeCursor(frame.cursor); const values = boundedArray(frame.sessions, "sessions"); for (let i = 0; i < values.length; i++) decodeSessionRef(values[i], `sessions[${i}]`); return { ...frame, cursor } as unknown as SessionsFrame; }
