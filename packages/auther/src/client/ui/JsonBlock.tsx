...
export function JsonBlock({ data, title, initialCollapsed = false }: JsonBlockProps) {
	...
	// Clear the pending "Copied" reset if the block unmounts.
	...
	return (
		<div className="home-json-block">
			<div
				className="home-json-block-header"
			...
			>
				<span className="home-json-block-title">{title || "JSON"}</span>
				<div className="home-json-actions">
					...
					<span className="home-json-block-toggle-indicator" data-collapsed={collapsed}>
						{collapsed ? "▶ Show" : "▼ Hide"}
					</span>
				</div>
			</div>
			{!collapsed && (
				<div className="home-json-block-content-wrapper">
					<pre className="home-json-block-content">
						<code>{jsonStr}</code>
					</pre>
				</div>
			)}
