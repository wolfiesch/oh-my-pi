...
export function AsyncBoundary({
	...
	// If it's loading and there is no stale data, render the skeleton fallback.
	if (loading && data === null) {
		if (fallback) {
			return <>{fallback}</>;
		}
		return (
			<div className="home-boundary-skeleton">
	...
	// Render children (stale data is kept visible while a background refresh runs).
	return <>{children}</>;
}
