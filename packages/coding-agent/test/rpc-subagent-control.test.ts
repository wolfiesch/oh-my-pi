...
describe("cancelRpcSubagent", () => {
	...
	test("publishes an aborted state before releasing an idle subagent", async () => {
		const { registry, lifecycle } = registryAndLifecycle();
		const events: string[] = [];
		registry.register({ id: "IdleWorker", displayName: "Idle worker", kind: "sub", session: null, status: "idle" });
		registry.onChange(event => events.push(`${event.type}:${event.ref.status}`));

		await expect(cancelRpcSubagent("IdleWorker", registry, lifecycle)).resolves.toBe(true);
		expect(events).toEqual(["status_changed:aborted", "removed:aborted"]);
		expect(registry.get("IdleWorker")).toBeUndefined();
	});
