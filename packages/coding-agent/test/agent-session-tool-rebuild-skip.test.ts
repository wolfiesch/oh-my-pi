	...
	function newSession(
		...
		const session = new AgentSession({
			...
			extensionRunner: options.beforeAgentStartSystemPrompt
				...
				: undefined,
			rebuildSystemPrompt: async (toolNames, _tools, mountedXdevNames) => {
				const base = await rebuildSystemPrompt(toolNames);
				if (!options.exposeXdevCatalog) return { systemPrompt: [base] };
				const catalog = [...(mountedXdevNames ?? options.xdev?.mountedNames ?? [])];
...
- xd://mcp__nucleus_fetch — This is inline documentation, not an inventory entry.
	...
	it("publishes an xd:// device atomically with its lazy write transport", async () => {
		...
		expect([...xdevState.mountedNames]).not.toContain(search.name);
		expect(session.getMountedXdevToolNames()).not.toContain(search.name);
		expect(session.getActiveToolNames()).not.toContain("write");
		...
		expect([...xdevState.mountedNames]).toContain(search.name);
