...
export const SETTINGS_SCHEMA = {
	...
	// therefore persist in local/config.yml rather than the portable global file.
...
/** Host-local setting paths, derived from the schema for settings persistence. */
export const HOST_LOCAL_PATHS: readonly SettingPath[] = (Object.keys(SETTINGS_SCHEMA) as SettingPath[]).filter(path => {
	const definition = SETTINGS_SCHEMA[path];
	return "hostLocal" in definition && definition.hostLocal === true;
});

/** Check whether a setting belongs to the host-local layer. */
