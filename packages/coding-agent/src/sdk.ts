...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		const rebuildSystemPrompt = async (
			...
			const promptXdev = toolSession.xdev
				? { ...toolSession.xdev, mountedNames: new Set(mountedXdevNames ?? toolSession.xdev.mountedNames) }
				: undefined;
			...
			const projection = projectMountedMCPXdevGuidance(
				collectMountedMCPToolRoutes(promptXdev ? listXdevTools(promptXdev) : []),
			);
			...
			const defaultPrompt = await buildSystemPromptInternal({
				...
				xdevDocs: promptXdev
					? xdevDocsAll(promptXdev, settings.get("tools.xdevDocs"), settings.get("tools.xdevInlineDevices"))
					: "",
