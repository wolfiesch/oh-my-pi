...
interface SessionToolsOptions {
	...
	rebuildSystemPrompt?: (
		...
		mountedXdevNames?: ReadonlySet<string>,
...
export class SessionTools {
	...
	#lazyWriteRegistrationAttempted = false;
	#skills: Skill[];
	#skillWarnings: SkillWarning[];
	...
	async applyActiveToolsByName(toolNames: string[]): Promise<void> {
		const writeWasBuiltIn = this.#builtInToolNames.has("write");
		const previousWrite = this.#toolRegistry.get("write");
		...
		if (mountCandidates.length > 0 && !builtInWriteAvailable && !this.#lazyWriteRegistrationAttempted) {
			this.#lazyWriteRegistrationAttempted = true;
			builtInWriteAvailable = (await this.#ensureWriteRegistered?.()) === true;
			if (builtInWriteAvailable) this.#builtInToolNames.add("write");
		}
		...
		if (transportNeeded && !builtInWriteAvailable && !this.#lazyWriteRegistrationAttempted) {
			this.#lazyWriteRegistrationAttempted = true;
			builtInWriteAvailable = (await this.#ensureWriteRegistered?.()) === true;
			if (builtInWriteAvailable) this.#builtInToolNames.add("write");
		}
		...
		try {
			if (this.#rebuildSystemPrompt) {
				const signature = this.#computeAppliedToolSignature(validToolNames, tools, mountNames);
				if (signature !== this.#lastAppliedToolSignature) {
					let rebuild: Promise<{ systemPrompt: string[]; xdevCatalogNames?: readonly string[] }>;
					this.#setMountedNames(mountNames);
					this.#setActiveToolNames?.(validToolNames);
					try {
						rebuild = this.#rebuildSystemPrompt(validToolNames, this.#toolRegistry, mountNames);
					} finally {
			...
			if (!writeWasBuiltIn) {
				this.#builtInToolNames.delete("write");
				if (previousWrite) this.#toolRegistry.set("write", previousWrite);
				else this.#toolRegistry.delete("write");
			}
			throw error;
		}
	...
	async setActiveToolsByName(toolNames: string[]): Promise<void> {
		...
		// A direct runtime selection is explicit: every named tool stays top-level.
		// Snapshot restoration uses setActiveToolPresentation when it must preserve
		// an existing xd:// partition.
		await this.#applyToolPresentation(normalized, new Set(), this.getActiveToolNames().includes("write"));
	}
	...
	#computeAppliedToolSignature(
		toolNames: string[],
		tools: AgentTool[],
		mountedXdevNames: ReadonlySet<string> = this.#xdev?.mountedNames ?? new Set(),
	): string {
		...
		const xdevForProjection = this.#xdev ? { ...this.#xdev, mountedNames: new Set(mountedXdevNames) } : undefined;
		const mountedMCPProjection = projectMountedMCPXdevGuidance(
			collectMountedMCPToolRoutes(xdevForProjection ? listXdevTools(xdevForProjection) : []),
		);
