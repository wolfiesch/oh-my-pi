...
import { Process as NativeProcess, PtySession } from "@oh-my-pi/pi-natives";
...
async function secureReadFile(root: string, relativePath: string, maxBytes: number): Promise<SecureReadFileResult> {
	const { target } = await secureTarget(root, relativePath);
	const info = await fs.lstat(target);
	if (!info.isFile() || info.isSymbolicLink()) throw new Error("UNSAFE_PATH");
	if (info.size > maxBytes) throw new Error("BOUNDS");
	const data = await fs.readFile(target);
	if (data.byteLength > maxBytes) throw new Error("BOUNDS");
	return { data, revisionSha256: createHash("sha256").update(data).digest("hex"), size: data.byteLength };
}

async function secureListDirectory(
	root: string,
	relativePath: string | undefined | null,
	maxEntries: number,
): Promise<SecureListDirectoryResult> {
	const { root: canonicalRoot, target } = relativePath
		? await secureTarget(root, relativePath)
		: { root: await fs.realpath(root), target: await fs.realpath(root) };
...
async function secureWriteFileAtomic(
	root: string,
	relativePath: string,
	data: Buffer,
	expectedRevision: string | undefined | null,
	maxBytes: number,
): Promise<SecureWriteFileResult> {
	if (data.byteLength > maxBytes) throw new Error("BOUNDS");
	const { target } = await secureTarget(root, relativePath, true);
	...
	const revision = current ? createHash("sha256").update(current).digest("hex") : undefined;
	if (expectedRevision != null && revision !== expectedRevision) throw new Error("STALE_REVISION");
	...
	return { revisionSha256: createHash("sha256").update(data).digest("hex"), size: data.byteLength };
}
...
export class CodingAgentDesktopAuthority {
	...
	async catalogGet(): Promise<{ items: unknown[] }> {
		...
		return { items: values.map((value, index) => this.#metadata(catalogItem(value, index))) };
	}
