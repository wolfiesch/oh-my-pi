...
import { type AgentSessionEvent, type AgentSessionEventListener, correlateAgentEvent } from "./agent-session-events";
import type {
	AgentSessionConfig,
...
export class AgentSession {
	...
	/** Register a one-shot callback invoked synchronously when disposal starts. */
	onDispose(listener: () => void): () => void {
		...
		return () => {
			...
			if (index >= 0) this.#disposeListeners.splice(index, 1);
		};
	}
	...
	/**
	 * Accepted app follow-ups must not remain stranded behind a transcript tail
	 * that cannot be continued directly, such as a preserved advisor card.
	 */
