...
import { catalogId, decodeCatalog, type CatalogFrame, type CatalogItem, type SettingsFrame } from "@oh-my-pi/app-wire";
...
export interface SettingsReadArgs { paths?: string[]; path?: string; category?: string; }
export interface SettingsWriteEdit { path: string; value?: unknown; scope?: SettingScope | string; reset?: boolean; controlType?: string; type?: string; }
export interface SettingsWriteArgs extends Partial<SettingsWriteEdit> { edits?: SettingsWriteEdit[]; expectedRevision?: string; }
export interface CatalogGetArgs { kind?: string; search?: string; query?: string; }
interface SettingDefinition { type: SettingControlType; default?: unknown; values?: readonly string[]; ui?: Record<string, unknown>; [key: string]: unknown; }
...
function settingDefinition(path: string): SettingDefinition | undefined {
	...
	return typeof def.type === "string" && SETTING_TYPES.has(def.type) ? def as unknown as SettingDefinition : undefined;
}
function settingSensitive(path: string): boolean { return path.split(".").some(secretKey); }
...
function controlMetadata(def: SettingDefinition): Record<string, unknown> {
	...
	const ui = def.ui;
	if (ui && Array.isArray(ui.options)) result.options = ui.options.slice(0, 256).map(option => {
		if (!option || typeof option !== "object") return undefined;
		const item = option as Record<string, unknown>;
		return { value: safeMetadata(item.value), label: safeMetadata(item.label), description: safeMetadata(item.description) };
	}).filter(Boolean);
	for (const key of ["min", "max", "unit", "scopes", "restartRequired", "platform", "availability", "maxItems", "maxEntries"]) if (def[key] !== undefined) result[key] = safeMetadata(def[key]);
	return result;
}
...
function validateValue(def: SettingDefinition, value: unknown, path: string): void {
	...
	switch (def.type) {
		case "boolean": if (typeof value !== "boolean") throw new Error(`invalid boolean value for ${path}`); break;
...
export class DesktopConfigAuthority {
	...
	constructor(options: DesktopConfigAuthorityOptions) { if (!options?.settings) throw new Error("settings is required"); this.#settings = options.settings; this.#hostId = options.hostId ?? "desktop"; this.#platform = options.platform ?? process.platform; this.#options = options; }
	#settingItems(): CatalogItem[] {
		return Object.keys(SETTINGS_SCHEMA).sort().flatMap(path => {
			const def = settingDefinition(path); if (!def) return [];
			const ui = def.ui; const sensitive = settingSensitive(path); const configured = this.#settings.isConfigured?.(path as SettingPath) ?? false;
			const metadata: Record<string, unknown> = { path, label: text(ui?.label) ?? path, description: text(ui?.description), ...controlMetadata(def), default: sensitive ? undefined : safeMetadata(def.default), effective: sensitive ? undefined : safeMetadata(this.#settings.get(path as SettingPath)), effectiveSource: sourceFor(this.#settings, path as SettingPath), configured, sensitive, scopes: ["global", "session"], platform: this.#platform, availability: this.#platform === "darwin" || path !== "power.sleepPrevention", ...(ui?.tab ? { tab: ui.tab } : {}), ...(ui?.group ? { group: ui.group } : {}) };
			return [{ id: catalogId(`setting:${path}`), kind: "setting", name: path, description: text(ui?.description), metadata: safeMetadata(metadata) as Record<string, unknown> }];
		});
	}
	...
	async catalogGet(args: CatalogGetArgs = {}, _context?: OperationContextLike): Promise<CatalogFrame> {
		const deduped = new Map<string, CatalogItem>(); for (const item of await this.#catalogRaw()) { if (!CATALOG_KINDS.has(item.kind) || deduped.has(item.id)) continue; const clean = safeMetadata(item) as Record<string, unknown>; if (typeof clean.id === "string" && typeof clean.name === "string") deduped.set(item.id, clean as unknown as CatalogItem); }
		const needle = (args.search ?? args.query)?.toLowerCase(); let items = [...deduped.values()]; if (args.kind) items = items.filter(item => item.kind === args.kind); if (needle) items = items.filter(item => `${item.name} ${item.description ?? ""} ${item.id}`.toLowerCase().includes(needle)); items.sort((a, b) => a.kind.localeCompare(b.kind) || a.name.localeCompare(b.name) || a.id.localeCompare(b.id));
		const frame = decodeCatalog({ v: "omp-app/1", type: "catalog", hostId: this.#hostId, revision: this.#revision(), items: items.slice(0, MAX_ITEMS) }); if (frame.type !== "catalog") throw new Error("catalog decoder returned settings frame"); return frame;
	}
	async configWrite(_args: unknown): Promise<never> { throw new Error("config.write is unsupported; use validated settings.write"); }
}
export function createDesktopConfigAuthority(options: DesktopConfigAuthorityOptions): DesktopConfigAuthority { return new DesktopConfigAuthority(options); }
