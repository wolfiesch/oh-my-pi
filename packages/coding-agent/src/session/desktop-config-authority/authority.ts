...
const COMMAND_DESCRIPTORS: Record<string, { capability: string; description: string }> = {};
const DESKTOP_CATALOG_COMMANDS: readonly string[] = [];
function catalogId(value: string): string {
	if (!value || Buffer.byteLength(value, "utf8") > 256) throw new Error("catalog id is invalid");
	return value;
}
