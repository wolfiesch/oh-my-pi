...
export function decodeOwnerRecord(value: unknown): OwnerRecord {
  ...
  if (!exactKeys(record, ["backingName", "device", "inode", "ownerId", "pid", "version"])) throw new Error("malformed appserver owner record");
  if (record.version !== 2 || typeof record.ownerId !== "string" || !UUID.test(record.ownerId) || !validInteger(record.pid, 1) || typeof record.backingName !== "string" || !UUID.test(record.backingName.slice(".appserver-".length, -".sock".length)) || record.backingName !== `.appserver-${record.ownerId}.sock` || !validInteger(record.device, 0) || !validInteger(record.inode, 0)) throw new Error("malformed appserver owner record");
  return { version: 2, ownerId: record.ownerId, pid: record.pid, backingName: record.backingName, device: record.device, inode: record.inode };
}
...
export function ownerPaths(publicPath: string, ownerId: string): OwnerPaths {
  ...
  return { directory, ownerPath: `${publicPath}.owner`, backingPath: join(directory, backingName), backingName, publicPath };
}

export function sameIdentity(a: Pick<OwnerRecord, "device" | "inode">, b: Pick<OwnerRecord, "device" | "inode">): boolean {
...
export async function ensureSecureSocketDirectory(publicPath: string): Promise<string> {
  ...
  if (info.isSymbolicLink() || !info.isDirectory()) throw new Error(`appserver socket directory is not a secure directory: ${directory}`);
...
export async function readStrictOwner(ownerPath: string): Promise<{ record: OwnerRecord; stat: Stats }> {
  ...
  if (first.isSymbolicLink() || !first.isFile() || (first.mode & 0o777) !== 0o600) throw new Error("malformed appserver owner marker");
  const text = await readFile(ownerPath, "utf8");
  const second = await lstat(ownerPath);
  if (first.dev !== second.dev || first.ino !== second.ino) throw new Error("appserver owner marker changed during read");
  let parsed: unknown;
  try { parsed = JSON.parse(text); } catch { throw new Error("malformed appserver owner marker"); }
  return { record: decodeOwnerRecord(parsed), stat: first };
}

export async function readPublicTarget(publicPath: string): Promise<{ stat: { device: number; inode: number }; target: string }> {
  ...
  if (isAbsolute(target) || target.includes("/") || target.includes("\\") || target === "." || target === ".." || relative(dirname(publicPath), resolve(dirname(publicPath), target)) !== target) throw new Error("appserver public symlink target is unsafe");
