...
export function normalizeIpAddress(address: string): string {
  return address.startsWith("::ffff:") && isIP(address) === 6 && isIP(address.slice(7)) === 4 ? address.slice(7) : address;
}
...
function immutablePeer(peer: ListenerPeerContext): ListenerPeerContext {
  const identity: RemotePeerIdentity = Object.freeze({ ...peer.identity, addresses: Object.freeze([...peer.identity.addresses]) });
...
export class BunRemoteListener {
  ...
  start(): void {
    ...
    run.server = Bun.serve<SocketData>({
      ...
      fetch: async (request, server) => {
        ...
        if (!originAllowed(request.headers.get("origin"), this.config.originAllowlist)) return new Response("Forbidden", { status: 403 });
        ...
        try {
          ...
          if (!server.upgrade(request, { data: { peer: immutablePeer(peer), connectionId, reserved: true, opened: false } }))
      ...
      websocket: {
        ...
        message: (ws, message) => {
          ...
          try {
            const result = this.hooks.message?.(entry.connection, typeof message === "string" ? message : new Uint8Array(message));
  ...
  private finish(run: RunState, entry: SocketEntry): void {
