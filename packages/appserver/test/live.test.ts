...
describe("live Unix websocket protocol", () => {
	...
	test("routes agent cancellation only to the target session RPC child and reflects its result", async () => {
		...
		try {
			...
			expect(first.writes.map(value => JSON.parse(value)).some(frame => frame.type === "cancel_subagent")).toBe(false);
			second.push({
				type: "response",
			...
			expect((await untilResponse(client.client, "cancel-true")).response).toMatchObject({
				ok: true,
			...
			client.client.sendJson(
				confirmFrame("cancel-false-confirm", String(falseChallenge.confirmationId), "cancel-false", "approve", "s2"),
			);
			...
			client.client.sendJson(
				confirmFrame("cancel-error-confirm", String(errorChallenge.confirmationId), "cancel-error", "approve", "s2"),
			);
