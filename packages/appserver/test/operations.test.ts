...
import { DEVICE_CAPABILITIES, commandId, decodeCommand, hostId, requestId, revision, sessionId, terminalId } from "@oh-my-pi/app-wire";
...
const context: OperationContext = { hostId: hostId("host-1"), sessionId: sessionId("session-1"), deviceId: "device-1", connectionId: "connection-1", capabilities: new Set(DEVICE_CAPABILITIES), currentRevision: revision("r-1"), abortSignal: new AbortController().signal };
...
function authority(overrides: Partial<DesktopOperationsAuthority> = {}): DesktopOperationsAuthority { return { filesRead: async () => ({ content: "hello" }), filesList: async () => ({ entries: [] }), filesDiff: async () => ({ diff: "" }), filesWrite: async () => ({}), filesPatch: async () => ({}), reviewRead: async () => ({}), reviewApply: async () => ({}), agentCancel: async () => ({ cancelled: true }), bashRun: async () => ({}), termOpen: async () => ({ terminalId: "term-1" }), catalogGet: async () => ({ items: [] }), settingsRead: async () => ({ revision: "revision-settings", settings: {} }), settingsWrite: async () => ({}), configWrite: async () => ({}), previewLaunch: async () => ({}), previewState: async () => ({}), previewNavigate: async () => ({}), previewCapture: async () => ({ content: "" }), terminalInput: async () => {}, terminalResize: async () => {}, terminalClose: async () => {}, ...overrides }; }
...
describe("desktop operation dispatcher", () => {
	...
	test("terminal owner registry rejects wrong connection and releases on disconnect", () => {
		...
		const owner = { connectionId: context.connectionId, deviceId: context.deviceId, hostId: context.hostId, sessionId: context.sessionId!, terminalId: terminalId("term-1") };
	...
	test("disconnect closes every owned terminal and late callbacks remain harmless", async () => {
		...
		const dispatcher = new DesktopOperationDispatcher(authority({
			...
			terminalClose: async () => { closes++; if (closes === 1) throw new Error("close failed"); },
		}));
		await dispatcher.dispatch(command("term.open"), context);
		await dispatcher.dispatch({ ...command("term.open"), commandId: commandId("command-term-open-2"), requestId: requestId("request-term-open-2") }, context);
		await expect(dispatcher.disconnect(context.connectionId, { ...context, sessionId: context.sessionId! })).rejects.toMatchObject({ code: "OPERATION_FAILED" });
		expect(closes).toBe(2);
	});
	test("abortSignal identity is passed unchanged to authority", async () => {
		let seen: AbortSignal | undefined;
		const signal = new AbortController().signal;
		const dispatcher = new DesktopOperationDispatcher(authority({ filesRead: async (_args, operationContext) => { seen = operationContext.abortSignal; return { content: "ok" }; } }));
